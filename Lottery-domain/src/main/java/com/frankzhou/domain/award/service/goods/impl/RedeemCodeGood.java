...
@Slf4j
@Component
public class RedeemCodeGood extends AbstractDistributionBase implements IDistributionGoods {
    @Override
    public DistributionRes doDistribution(GoodsReq goodsReq) {

        log.info("模拟调用优惠卷服务(GoodMircoService)给用户:{} 发送兑换码:{}",goodsReq.getUId(),goodsReq.getAwardContent());
