...
@Slf4j
@Component
public class CouponGood extends AbstractDistributionBase implements IDistributionGoods {
    @Override
    public DistributionRes doDistribution(GoodsReq goodsReq) {

        log.info("模拟调用优惠卷服务(CouponMircoService)给用户:{} 发送优惠卷:{}",goodsReq.getUId(),goodsReq.getAwardContent());
